...
function mapUser(u) {
  ...
  return {
    ...
    // Student-owned preference, not an access gate. `!== false` means a missing
    // column (migration not yet run) reads as "on", matching the DB default.
    reviewRemindersEnabled: u.review_reminders_enabled !== false,
    level:             u.level || 1,
    xp:                u.xp || 0,
