...
module.exports = function createEvalQueue({
  ...
  async function runEvalJob(job) {
    ...
    try {
      ...
      // (<20 chars or >90% copy of the question text). We still flow through the
      // model-answer resolution path below so the student sees the properly
      // formatted ALAC card Model Answer alongside their 0/10 — that's how
      // they learn what a correct answer looks like.
      let _trivialResult = null;
      let _skipScoringCall = false;
      if (isTrivialNonAnswer(answer, question)) {
        ...
        if (isSit) {
          _trivialResult = {
            ...
            writingFeedback: { spelling: [], grammar: [], overall: 'Answer is too brief to assess writing quality — write a complete response first.' },
...
In addition to scoring, identify spelling errors, grammar issues, and general writing observations in the student's answer. This is SEPARATE from scoring — do not deduct ALAC points for mechanics issues. Populate the writingFeedback field:
- spelling[]: list specific misspellings, format "'wrong' should be 'correct'" (empty array if none)
- grammar[]: list specific grammar issues, e.g. subject-verb agreement, tense inconsistency, punctuation (empty array if none)
- overall: one encouraging sentence summarizing writing quality (e.g. "Clear, well-constructed response with no significant errors.")
Keep each spelling/grammar entry under 25 words. Keep overall under 30 words.

Respond ONLY with valid JSON: {"score":"X/10","numericScore":0,"grade":"...","alac":{"answer":{"score":0,"max":1.5,"feedback":"under 50 words — start with what student got right","studentDid":""},"legalBasis":{"score":0,"max":3,"feedback":"under 50 words — start with what student got right","studentDid":""},"application":{"score":0,"max":4,"feedback":"under 50 words — start with what student got right","studentDid":""},"conclusion":{"score":0,"max":1.5,"feedback":"under 50 words — start with what student got right","studentDid":""}},"overallFeedback":"Open with what student did well then areas to strengthen then encouraging close. Under 200 words","strengths":[],"improvements":["Consider discussing...","A stronger answer would also include..."],"keyMissed":["A complete answer would address...","Consider also discussing..."],"writingFeedback":{"spelling":[],"grammar":[],"overall":"one encouraging sentence about writing quality"},"matchedAlternative":1,"format":"essay"}`;
      } else {
        maxTok = 2500;
...
Respond ONLY with valid JSON: {"score":"X/10","numericScore":0,"grade":"...","breakdown":{"accuracy":{"score":0,"max":4,"feedback":"under 50 words — start with what student got right"},"completeness":{"score":0,"max":3,"feedback":"under 50 words — start with what student got right"},"clarity":{"score":0,"max":3,"feedback":"under 50 words — start with what student got right"}},"overallFeedback":"Open with what student did well then areas to strengthen then encouraging close. Under 100 words","keyMissed":["A complete answer would address...","Consider also discussing..."],"strengths":[],"improvements":["Consider discussing...","A stronger answer would also include..."],"writingFeedback":{"spelling":[],"grammar":[],"overall":"one encouraging sentence about writing quality"},"matchedAlternative":1,"format":"conceptual"}`;
      ...
      //
      // When _skipScoringCall is set (trivial-input short-circuit), we still
      // fire the model-answer generation so the student sees the formatted
      // ALAC Model Answer card, but skip the Haiku scoring call entirely.
      const needsAlacGen       = isSit  && altCount === 0 && !_cachedAlac && !!modelAnswer;
      const needsConceptualGen = !isSit && !_cachedConceptual && !!modelAnswer;

      const scorePromise = _skipScoringCall
        ? Promise.resolve(_trivialResult)
        : callClaudeHaikuJSON(prompt, maxTok);
      ...
      if (result) {
        ...
        if (questionId && job.userId && !result._evalError) {
          ...
          // Canonical rule. This branched on `alac.answer.score != null` while
          // the total-score path branched on `alac` being truthy, so an
          // evaluation missing its Answer component was scored one way for the
          // record and another for mastery.
          const srScore = BBScore.questionScore(result);
