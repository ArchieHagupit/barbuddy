...
//
// Mastery criteria (either triggers mastery; mastered cards are filtered
// out of the due queue so students don't see them again):
//   (A) rating === 'easy'  — any easy rating masters the card immediately
//   (B) intervalDays >= 60 AND reviewCount >= 5 AND rating in ['good','easy']
//       — long-interval safety net for cards that graduate gradually
...
function updateState(prev, rating, { now = new Date() } = {}) {
  ...
  // Mastery triggers:
  //   (1) Aggressive rule — any 'easy' rating masters the card immediately.
  //       Rationale: 'easy' means "I know this cold, don't show me again."
  //       To switch to "require >=2 reviews before easy can master", change
  //       the line below to: rating === 'easy' && reviewCount >= 2
  //   (2) Conservative rule — long interval + many reviews + clean recall.
  const masteredByEasy =
    rating === 'easy';
  const masteredByLongInterval =
    intervalDays >= MASTERY_INTERVAL_DAYS &&
    reviewCount >= MASTERY_MIN_REVIEWS &&
    (rating === 'good' || rating === 'easy');
  const mastered = masteredByEasy || masteredByLongInterval;
