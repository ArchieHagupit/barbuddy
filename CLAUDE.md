...
## Alternative Answer ALAC Cache

### Columns (questions table)
- alternative_alac_1 through alternative_alac_5 (JSONB)
- Stores ALAC components for each alternative answer
- Generated by generateAlternativeAlac() which 
  reuses generateALACModelAnswer() internally

### Cache-First Logic
1. Check _qCache.alternativeAlacN for matched alt
2. If null → generate via generateAlternativeAlac()
3. Fire-and-forget write to alternative_alac_N
4. Applied in both /api/evaluate and runEvalJob()

### Cache Invalidation
- model_answer edited → clears all 5 alt ALAC columns
- alternative_answer_N edited → clears only 
  corresponding alternative_alac_N

### Backfill
- POST /api/admin/backfill-alternative-alac
- GET /api/admin/backfill-alternative-alac/status
- Generates ALAC for all non-null alternative answers 
  missing their ALAC cache
- Admin button: "Pre-generate Alternative ALAC" (orange)

### Display Priority in Results
1. matchedAlternativeAlac (if alternative matched)
   Label: "Alternative Answer N (ALAC Format)"
2. model_answer_alac (if no alternative matched)
   Label: "Model Answer (ALAC Format)"
3. Plain text fallback
